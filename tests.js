...
test("By default, maximum precision is kept, but it can be changed later", function () {
  ...
  equal(d.getPrecision(), MSEC);
  d.setPrecision(SEC);
  equal(d.getPrecision(), SEC);
  d.setPrecision(DAY);
  equal(d.getPrecision(), DAY);
  d.setPrecision(MONTH);
  equal(d.getPrecision(), MONTH);
});
...
test("Display timestamp value trucated to precision", function () {
  ...
  strictEqual(d.toPrecisionString(MSEC), '2012-03-04 09:52:13.746');
  strictEqual(d.toPrecisionString(SEC), '2012-03-04 09:52:13');
  strictEqual(d.toPrecisionString(MIN), '2012-03-04 09:52');
  strictEqual(d.toPrecisionString(HOUR), '2012-03-04 09');
  strictEqual(d.toPrecisionString(DAY), '2012-03-04');
  strictEqual(d.toPrecisionString(MONTH), '2012-03');
  strictEqual(d.toPrecisionString(YEAR), '2012');
